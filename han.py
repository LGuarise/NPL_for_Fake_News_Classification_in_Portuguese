...
def han2(MAX_NB_WORDS, MAX_WORDS, MAX_SENTS, EMBEDDING_DIM, WORDGRU, embedding_matrix, DROPOUTPER):
    ...
    wordEmbedding = Embedding(MAX_NB_WORDS, EMBEDDING_DIM, weights=[embedding_matrix], mask_zero=True, trainable=False, name='wordEmbedding')(wordInputs) 
    
    hij = Bidirectional(LSTM(WORDGRU, return_sequences=True), name='gru1')(wordEmbedding)
    ...
    hi = Bidirectional(LSTM(WORDGRU, return_sequences=True), merge_mode='concat', name='gru2')(sentEncoding)
